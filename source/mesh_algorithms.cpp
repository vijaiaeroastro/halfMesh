...
bool Mesh::is_multiply_connected() const {
    ...
    while (!stack.empty()) {
        ...
        // walk all three half-edges of f
        HalfEdgePtr he = f->get_one_half_edge();
        for (int i = 0; i < 3 && he; ++i) {
            if (i>0) he = get_next_half_edge(he, f);
            unsigned opp = he->get_opposing_half_edge();
            if (opp == std::numeric_limits<unsigned>::max()) continue;
            HalfEdgePtr he2 = get_half_edge(opp);
            FacePtr     nf  = get_face(he2->get_parent_face());
